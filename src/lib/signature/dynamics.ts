/**
 * dynamics.ts — Live-State-Engine für die dynamische Signatur
 * (Port von shared/dynamics.js aus DYAI2025/bazodiac-signature-prototypes,
 * DYNAMIC_SIGNATURE_CONCEPT.md Layer 1 + 2).
 *
 * Pollt echtes Space Weather (NOAA SWPC planetarer Kp-Index) und emittiert
 * periodisch einen Zustand, der via applyDynamics() (signatureData.ts) auf
 * die Eingangsvektoren angewendet wird. Fällt der Feed aus, wird NIE
 * geworfen, sondern ein klar als 'simuliert' markierter, deterministischer
...
import { ELEMENT_ORDER, type DynamicsState } from "./signatureData";
...
async function fetchCosmicWeather(): Promise<{ normalized: number; kp: number; source: "noaa" }> {
 ...
 */
function simulatedCosmicWeather(nowMs: number): { normalized: number; source: "simuliert" } {
 ...
 * aktuelle Datum (Perioden 9–25 Tage, Phase über den Element-Index geseedet),
 * Amplitude 0.06 (hart unter dem Transit-Bound ±0.10).
 * Platzhalter, bis FuFirE /v1/transit/state angebunden ist.
 */
function simulatedTransitDeltas(nowMs: number): { western: Record<string, number> } {
...
export interface DynamicsHandle {
  stop: () => void;
}
...
export function startDynamics({
  ...
  onState: (state: DynamicsState) => void;
  intervalMs?: number;
}): DynamicsHandle {
  let stopped = false;
  ...
  async function tick() {
    ...
    let cosmicWeather: DynamicsState["cosmic_weather"];
    try {
      cosmicWeather = await fetchCosmicWeather();
    } catch (err) {
      console.warn("[dynamics] NOAA-Feed nicht erreichbar, simulierter Fallback:", err instanceof Error ? err.message : err);
    ...
    try {
      ...
      console.warn("[dynamics] onState-Callback hat geworfen:", err instanceof Error ? err.message : err);
    }
  }
