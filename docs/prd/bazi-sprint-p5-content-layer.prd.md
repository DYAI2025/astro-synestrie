...
- **B — Engagement-Signal:** leichtes `card_click`/`layer_open`-Analytics-Event (kein Drittanbieter, keine PII), in Task 3 verdrahtet, Beta-Smoke-Ziel ≥1 Layer-Open, benannter Owner. Adressiert in **REQ-P5-009** (eigene REQ). Code-Read 2026-06-13: kein Analytics-Mechanismus in `src/` vorhanden (`belegt`) → neues, internes Modul nötig.
...
- **E — Astro-Noctum-Pivot-Gate:** fehlen für ≥2 ganze Domänen Quelltexte → „nicht wie spezifiziert fortfahren", an User eskalieren (kein stilles Erfind-durch-Umlabeln). Adressiert in **REQ-P5-001** + **NFR-06**.
...
**Pivot-Gate (Amendment E, bindend):** Findet Task 1 für **≥2 ganze Domänen** (von: zodiac, stems, branches, elements, pillars, houses) keine Quelltexte, ist das ein „nicht wie spezifiziert fortfahren"-Zustand → STOP + Eskalation an den User. Kein stiller „Executor kuratiert alle 55 aus dem Kopf"-Sprint, kein Umlabeln.
...
- [ ] **Given** ≥2 ganze Domänen ohne Quelltexte, **When** Task 1 das feststellt, **Then** stoppt der Sprint und ein User-Report mit den konkret fehlenden Domänen wird erstellt — KEIN Fortfahren mit Voll-Kuratierung. — *Test (falsifizierend): Pivot-Gate-Checklist-Eintrag im PR; Abwesenheit = Verstoß. Reviewer falsifiziert durch Abgleich `content-sources.md` `fehlt`-Count gegen die Eskalations-Notiz.*
...
- canvas-risk-status: "GATE — bei ≥2 fehlenden Domänen Pivot/Eskalation; risk aligned nur wenn Gate passiert oder eskaliert"
...
**Beta-Smoke-Ziel:** Der Beta-Smoke zeigt ≥1 Layer-Open (Verhaltens-Smoke, nicht nur Existenz). **Owner:** Das Engagement-Signal hat einen **benannten Owner** — dieser ist im PR/Abschlussbericht namentlich festzuhalten (Team-Lücke geschlossen). *(OPEN QUESTION an den User: Wer ist der benannte Owner? — siehe openGaps; PRD-Finalisierung benötigt diesen Namen.)*
...
- Pivot-Gate: ≥2 ganze Domänen ohne Quelltexte → STOP + User-Eskalation (kein stilles Erfind-durch-Umlabeln). Verankert in REQ-P5-001.
...
Fehlen für ≥2 Domänen Quelltexte, droht „Executor kuratiert 50 Texte aus dem Kopf" = Erfind-durch-Umlabeln. **Mitigierung:** REQ-P5-001 Pivot-Gate eskaliert an User statt zu verschlucken.
...
|---|---|---|---|
| **Task 1** | REQ-P5-001 | Quell-Exploration: `content-sources.md` (Quell-Mapping je Domäne, `grep`-verifiziert) + Astro-Noctum-Pivot-Gate (≥2 Domänen fehlen → Eskalation). (Hidden-Stems brauchen KEIN Real-Engine-Gate — bereits durch `realshapes.test.ts:103` belegt, siehe REQ-P5-007.) | **HARD GATE vor Task 2 und jedem Text** |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| REQ-P5-001 | content-sources.md Struktur + Pivot-Gate-Checklist | Task 1 | committetes `docs/contracts/content-sources.md` + Eskalations-Notiz bei ≥2 fehlenden Domänen | nein (Doc-Gate) | integration-fake | docs/canvas/bazi-sprint-p5-content-layer.canvas.md | Karten ohne Erklärung; App nur mit Vorwissen sinnvoll | Neugierige Erwachsene 25–45, Entertainment/Reflexion | Texte aus Astro-Noctum portiert/kuratiert, nie erfunden | content-sources.md existiert (Typ→Quelle→Qualität) | aligned | TBD | VC-P5-01 | offen |
...
| REQ-P5-009 | Unit Event-Spy + e2e + Beta-Smoke ≥1 Layer-Open + Owner-Doc | Task 3 | Event-Spy-Test, real-boundary-Beta-Smoke (≥1), Owner-Name im PR | ja (Event live) | real-boundary-smoke | docs/canvas/bazi-sprint-p5-content-layer.canvas.md | Alle Erfolgssignale test-grün; nichts misst, ob Karten geklickt werden | Neugierige Erwachsene 25–45, Entertainment/Reflexion | card_click/layer_open misst, ob Layer genutzt wird (Nutzen, nicht Existenz) | Event feuert; Beta-Smoke ≥1 Layer-Open; benannter Owner | value-risk | TBD | VC-P5-09 | offen — Owner-Name OPEN QUESTION |
...
1. **REQ-P5-009 — Owner des Engagement-Signals (OPEN QUESTION):** Amendment B verlangt einen *benannten* Owner. Der Canvas markiert die Team-Lücke als „geschlossen" über das Prinzip, nennt aber keinen Namen. → User muss den Owner-Namen liefern.
